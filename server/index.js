// server/index.js  –  bitte 1-zu-1 ersetzen
...
/* ──────────── ADMIN – steht ABSICHTLICH ganz zuoberst! ──────────── */
app.get("/admin/backup", (req, res) => {
  fs.access(DB_PATH, fs.constants.R_OK, (err) => {
    if (err) return res.status(404).json({ message: "DB nicht gefunden" });
...
app.post("/admin/restore", upload.single("file"), (req, res) => {
  ...
  fs.mkdir(DB_DIR, { recursive: true }, () => {
    fs.copyFile(req.file.path, DB_PATH, (err) => {
      ...
      res.json({ message: "Restore OK" });
    });
  });
});
/* ──────────────────────────────────────────────────────────────── */

/* Statischer React-Build – darf erst JETZT kommen */
const STATIC_DIR = path.join(__dirname, "..", "dist", "assets");
app.use(express.static(STATIC_DIR));

/* SPA-Fallback (index.html) – wirklich ganz zuletzt */
app.get("*", (_req, res) =>
  res.sendFile(path.join(STATIC_DIR, "index.html"))
);

app.listen(PORT, () =>
  console.log(`[BoxTracking] Läuft auf ${PORT} | DB → ${DB_PATH}`)
);
